    ...
    methods: {
        async openModal() {
            try {
                ...
                showErrorInfo(err?.message || String(err))
            }
        },
        dtClear() {
        ...
        isAdmin(group) {
            if (!this.currentUserJID) return false;

            // Check if current user is the owner (compare full JIDs)
            if (group.OwnerJID === this.currentUserJID) {
                return true;
            }

            // Strip device number and domain for loose comparison
            const myUser = this.currentUserJID.split('@')[0].split(':')[0];

            // Check if current user is an admin in participants
            const participant = group.Participants && group.Participants.find(p => {
                const pUser = (p.JID || '').toString().split('@')[0].split(':')[0];
                return pUser === myUser;
            });
