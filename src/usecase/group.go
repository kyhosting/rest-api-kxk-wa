...
func (service serviceGroup) SetGroupPhoto(ctx context.Context, request domainGroup.SetGroupPhotoRequest) (pictureID string, err error) {
	...
	if request.Photo != nil {
		...
		logrus.Infof("Processing group photo: %s (size: %d bytes)", request.Photo.Filename, request.Photo.Size)

		processedImageBuffer, err := utils.ProcessGroupPhoto(request.Photo)
		if err != nil {
			logrus.Errorf("Failed to process group photo: %v", err)
			return pictureID, err
		}

		logrus.Infof("Successfully processed group photo: %d bytes -> %d bytes",
	...
	if err != nil {
		logrus.Errorf("Failed to set group photo: %v", err)
		return pictureID, err
	}
