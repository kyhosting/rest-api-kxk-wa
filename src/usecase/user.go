...
func (service serviceUser) Info(ctx context.Context, request domainUser.InfoRequest) (response domainUser.InfoResponse, err error) {
        ...
        for jid, userInfo := range resp {
                ...
                data := domainUser.InfoResponseData{
                        ...
                        StatusAt:  userInfo.StatusAt.Unix(),
                ...
                if userInfo.VerifiedName != nil {
                        if userInfo.VerifiedName.Details != nil {
                                data.VerifiedName = userInfo.VerifiedName.Details.GetVerifiedName()
                        }
...
func (service serviceUser) BusinessProfile(ctx context.Context, request domainUser.BusinessProfileRequest) (response domainUser.BusinessProfileResponse, err error) {
        ...
        if profile.ProfileOptions != nil {
                ...
                // Extract well-known profile_options fields
                if v := profile.ProfileOptions["member_since_text"]; v != "" {
                        response.MemberSince = v
                }
                if v := profile.ProfileOptions["description"]; v != "" {
                        response.Description = v
                }
                if v := profile.ProfileOptions["website"]; v != "" {
                        response.Website = v
                }

                // Try to find tier from various possible keys
                tierKeys := []string{"tier", "business_tier", "biz_tier", "verified_tier", "level", "verified_level"}
                for _, key := range tierKeys {
                        if v, ok := profile.ProfileOptions[key]; ok && v != "" {
                                response.Tier = v
                                break
                        }
                }

                // Try to find cover ID from various possible keys
                coverKeys := []string{"cover_low_res_url", "cover_high_res_url", "cover_id", "cover", "biz_cover_photo"}
                ...
                // Try to find business name from profile_options
                nameKeys := []string{"business_name", "name", "display_name"}
                for _, key := range nameKeys {
                        if v, ok := profile.ProfileOptions[key]; ok && v != "" {
                                response.BusinessName = v
                                break
                        }
                }
        }
