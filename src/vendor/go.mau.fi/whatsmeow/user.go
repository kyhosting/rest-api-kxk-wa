...
func (cli *Client) GetUserInfo(ctx context.Context, jids []types.JID) (map[types.JID]types.UserInfo, error) {
        ...
        for _, child := range list.GetChildren() {
                ...
                statusNode := child.GetChildByTag("status")
                status, _ := statusNode.Content.([]byte)
                info.Status = string(status)
                if tsVal, ok := statusNode.Attrs["t"]; ok {
                        switch v := tsVal.(type) {
                        case int64:
                                info.StatusAt = time.Unix(v, 0)
                        case int:
                                info.StatusAt = time.Unix(int64(v), 0)
                        case uint64:
                                info.StatusAt = time.Unix(int64(v), 0)
                        }
                }
